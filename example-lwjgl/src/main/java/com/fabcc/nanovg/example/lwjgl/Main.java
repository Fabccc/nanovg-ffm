...
public class Main {
    ...
    private NVGDemo demo;
    private SlicingArena slicingArena;
    ...
    private void init() {
        ...
        this.nvgContext = NVG.nvgCreateContext(NVG.NVG_STENCIL_STROKES() | NVG.NVG_DEBUG());
        if (nvgContext.equals(MemorySegment.NULL)) {
            throw new RuntimeException("Could not init NanoVG context.");
        }
        this.demo = new NVGDemo(arena, "NanoVG FFM", nvgContext);

        long size = 2 * 1024;
        var slicingArenaBase = arena.allocate(size, 8);
        this.slicingArena = new SlicingArena(slicingArenaBase, size);
    }
    ...
    private void loop() {
        ...
        try (MemoryStack stack = stackPush()) {
            ...
            while (!glfwWindowShouldClose(window)) {
                ...
                NVG.nvgBeginFrame(nvgContext, this.windowWidth, this.windowHeight, 1.0f);
                demo.renderDemoLowMemalloc(slicingArena, cx, cy, this.windowWidth, this.windowHeight, time);
                slicingArena.reset();
